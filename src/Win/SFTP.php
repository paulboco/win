...
class SFTP {
	 ...
	 *
	 * The $connection array is associative and structured as follows.
	 *
	 *     array(
	 *         'host'     => '123.54.874.12',
	 *         'username' => 'root',
	...
	{
		static::$_host    = $connection['host'];
		static::$_user    = $connection['username'];
		static::$_pwd     = $connection['password'];
		static::$_port    = (int)$connection['port'];
		static::$_timeout = (int)$connection['timeout'];
		static::$passive  = $connection['passive'];
	...
	public function download($remote_file, $local_file, $mode = FTP_ASCII)
	{
		...
		if ($this->connect())
		{
			...
			// Download remote file and save as local file.
			if ($this->get($remote_file, $local_file, $mode))
			{
				print "File '{$remote_file}' was successfully downloaded\n";
			}
			else
			{
				print "Download failed: " . $this->error . "\n";
			}
		}
		else
		{
			// The connection failed, display the last error.
			print "Connection failed: " . $this->error . "\n";
		}
	}
